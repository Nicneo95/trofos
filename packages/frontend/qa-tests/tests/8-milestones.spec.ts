...
// Keep default mode so failures do not skip the remaining tests in this file
  ...
  milestoneTest('[Milestones] 8.1 - Create and Delete milestone', async ({ authenticatedPage: page }) => {
    ...
    await page.locator('.ant-form-item-control-input-content > .ant-picker').click();
    await page.waitForTimeout(300);
    // Pick first non-disabled date as start
    await page.locator('.ant-picker-cell:not(.ant-picker-cell-disabled) .ant-picker-cell-inner').first().click();
    await page.waitForTimeout(300);
    // Pick last non-disabled date as end (re-queried after first click)
    await page.locator('.ant-picker-cell:not(.ant-picker-cell-disabled) .ant-picker-cell-inner').last().click();
    await page.waitForTimeout(300);
    await page.getByRole('button', { name: 'Finish' }).click();
    await page.waitForTimeout(1000);
