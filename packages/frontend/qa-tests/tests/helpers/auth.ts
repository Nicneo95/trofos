...
export async function loginAsTestUser(page: Page): Promise<void> {
  ...
  // Wait for redirect to /projects — only happens when login succeeds
  await page.waitForURL('**/projects**', { timeout: 30000 }).catch(async () => {
    throw new Error(`Login failed or did not redirect. URL: ${page.url()}`);
  });
  ...
  // Ensure sidebar is rendered (userInfo loaded) before saving storage state
  await page.getByRole('menuitem', { name: /home/i }).first().waitFor({ state: 'visible', timeout: 45000 });
}
