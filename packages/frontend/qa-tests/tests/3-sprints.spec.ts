...
sprintTest('[Sprint] 3.3 - Sprint name required validation', async ({ authenticatedPage: page }) => {
  const newSprintBtn = page.getByRole('button', { name: 'New Sprint' });
  await newSprintBtn.waitFor({ state: 'visible', timeout: 15000 });
  await newSprintBtn.click();
  await page.waitForSelector('.ant-modal-content');
...
sprintTest('[Sprint] 3.4 - Sprint lifecycle: start, complete, retrospective', async ({ authenticatedPage: page }) => {
  ...
  // Wait for the RTK Query mutation to complete and re-render the button.
  // The "Complete Sprint" button has data-tour="complete-sprint-button" and is always visible
  // in the Collapse.Panel header (not inside collapsed content).
  // We deliberately avoid a page.reload() here because that can race the in-flight API call.
  await expect(
    page.locator('[data-tour="complete-sprint-button"]')
