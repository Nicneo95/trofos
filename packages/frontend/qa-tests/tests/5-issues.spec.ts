...
const issueTest = test.extend<{ authenticatedPage: Page }>({
  authenticatedPage: async ({ page }, use) => {
    ...
    // Wait for issues table to load (API-driven — may take a moment after DOM is ready)
...
issueTest('[Issues] 5.3 - Update comment on issue', async ({ authenticatedPage: page }) => {
  const initialText = `AutoTest initial comment ${Date.now()}`;
  ...
  // Add a comment first so this test is self-contained
  await page.getByRole('textbox', { name: 'Add new comment...' }).fill(initialText);
  await page.getByRole('button', { name: 'Add comment' }).click();
  await page.waitForTimeout(500);
  await expect(page.locator('.ant-list-item-meta-description', { hasText: initialText })).toBeVisible({ timeout: 5000 });
...
issueTest('[Issues] 5.4 - Delete comment on issue', async ({ authenticatedPage: page }) => {
  ...
  // Delete all comments (5.3 adds its own comment so there may be >1)
  for (let i = 0; i < 10; i++) {
...
issueTest('[Issues] 5.5 - Delete issue (cleanup)', async ({ authenticatedPage: page }) => {
  ...
  // Only delete rows created by this test suite ("AutoTest Issue *").
  // Deleting all rows would destroy board test issues running concurrently in the other worker.
  for (let safety = 0; safety < 20; safety++) {
    const autoTestRow = tabPanel.locator('tr').filter({ hasText: /AutoTest Issue/i }).first();
    if (!await autoTestRow.isVisible({ timeout: 2000 }).catch(() => false)) break;

    await autoTestRow.getByRole('button', { name: 'Delete' }).click();
    await page.getByRole('button', { name: 'OK' }).click();
    await page.waitForTimeout(1000);
  }

  // Verify no AutoTest issues remain
  await expect(tabPanel.locator('tr').filter({ hasText: /AutoTest Issue/i })).toHaveCount(0, { timeout: 8000 });
});
